...
def generate_trips(od_name, random_seed, run):
    ...
    trips_cmd = "od2trips -c ../od_data/aitm_baseline/%s_trip_config.cfg.xml -o ../trip_files/aitm_baseline/%s_run%s.trips.rou.xml --error-log ../od_data/aitm_baseline/logs/%s_run%s_od2trips_errors.txt --seed %s"  % (od_name, od_name, run_number, od_name, run_number, RANDOM_SEED)
...
def generate_routes(run_number):
    ...
    routes_cmd = "duarouter -c ../trip_files/aitm_baseline/%s_router.cfg.xml --route-files ../trip_files/aitm_baseline/%s_run%s.trips.rou.xml -o ../route_files/aitm_baseline/%s_run%s.rou.xml --error-log ../trip_files/aitm_baseline/logs/%s_run%s_duarouter_errors.txt" % (run_number, run_number, run_number)

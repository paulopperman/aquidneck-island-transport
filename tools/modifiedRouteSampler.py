...
# @date    2020-02-07
#
# Modified by Paul Opperman 2020-04-12
...
def get_options(args=None):
        ...
        try:
            ...
            if options.optimize != "full":
                try:
                    ...
                    #options.optimize = options.optimize  # for proportional bounding  # FIXME: switch back to int for documented functionality
                except Exception:
    ...
    if options.optimizeInput and type(options.optimize) != int:  # FIXME: switch back to int
    # if options.optimizeInput and type(options.optimize) != float:
...
def parseDataIntervals(parseFun, fnames, begin, end, routes, attr):
            ...
            if overlap > 0:
                ...
                for edges, value in parseFun(interval, attr):
                    if edges not in locations:
                        result.append(CountData(0, edges, routes))
...
def optimize(options, countData, allRoutes, usedRoutes, routeUsage):
    ...
    # FIXME: why is routeUsage not used in this function?
    ...
    # split routes into list of indices for calibrated and uncalibrated routes
    for rt in enumerate(routeUsage):
        if rt[1] != set():
            cal_routes.append(rt[0])
        else:
            uncal_routes.append(rt[0])
    ...
    pc = getRouteCounts(routes, usedRoutes)  # get all route counts
    priorRouteCounts = []  # filter route counts to calibrated counts
    for cr in cal_routes:
        priorRouteCounts.append(pc[cr])
    ...
    if options.optimize == "full":
        ...
        u = float(options.optimize)
        if u == 0:
            print("Optimization skipped")
            return
        # limited optimization: change prior routeCounts by at most u per route
        bounds = [(max(0, int(p - u)), int(p + u)) for p in priorRouteCounts] + [(0, None)] * m  # TODO: make this proportional to priorRouteCounts
        # limit optimization by u percent
        # bounds = [(max(0, int((1-u) * p), int((1+u) * p))) for p in priorRouteCounts] + [(0, None)] * m  # proportional bounds
    ...
    for i in range(0, m):
        for j in range(0, k):
            A[i][j] = int(cal_routes[j] in countData[i].routeSet)  # 1 if route passes the counter
    ...
    if res.success:
        ...
        cal_counts = res.x[:k]
        for cal_idx in range(0, len(cal_routes)):
            routeCounts[cal_routes[cal_idx]] = cal_counts[cal_idx]  # fill in calibrated counts
        for uncal_idx in range(0, len(uncal_routes)):
            routeCounts[uncal_routes[uncal_idx]] = pc[uncal_routes[uncal_idx]]  # fill in uncalibrated counts
          # cut of slack variables
        # slack = res.x[k:]
        ...
        usedRoutes.extend(sum([[i] * int(round(c)) for i, c in enumerate(routeCounts)], []))  # extends the array that was passed into the function
...
class RoutesSeries:
    def __init__(self, routefiles):
        ...
        for routefile in routefiles:
            self.rts += [tuple(r.edges.split()) for r in sumolib.xml.parse(routefile, 'route')]
            self.depart += [float(d.depart) for d in sumolib.xml.parse(routefile, 'vehicle')]
        # self.timed = sorted(tuple(zip(self.depart, self.rts)), key=lambda x: x[0])  # TODO: using a sorted list should help with an efficient algorithm, but pandas is easier
        self.timed = pd.DataFrame(tuple(zip(self.depart, self.rts)))

    def getRoutesForInterval(self, b, e):
        # select routes within timespan from b to e
        # return a single Routes object
        routeList = list(self.timed[(self.timed[0] >= b) & (self.timed[0] < e)][1])
        return Routes(routelist=routeList)


class Routes:
    def __init__(self, routefiles=None, routelist=[]):  #TODO: take in options to detect optimize-input
        self.all = routelist
        if routefiles is not None:
            for routefile in routefiles:
        ...
        self.edges2index = dict([(e, i) for i, e in enumerate(self.unique)])  # FIXME: is this really what it should be doing?  doesn't allow selecting based on enumeration
        self.loadedCounts = [0] * len(self.edges2index)  # route index to count
        for e in self.all:
            self.loadedCounts[self.edges2index[e]] += 1

    def buildFromRouteList(self, routeList):
        # build object without parsing file
        self.all = routeList
        self.unique = sorted(list(set(self.all)))
        self.number = len(self.unique)
...
def main(options):
    ...
    routesSeries = RoutesSeries(options.routeFiles)
    ...
    allRoutes = routesSeries.getRoutesForInterval(b, e)

    countData = (parseDataIntervals(parseTurnCounts, options.turnFiles, b, e, allRoutes, options.turnAttr)
                 + parseDataIntervals(parseEdgeCounts, options.edgeDataFiles, b, e, allRoutes, options.edgeDataAttr))
    routeUsage = getRouteUsage(allRoutes, countData)
    ...
    if options.verbose:
        print("Loaded %s routes (%s distinct)" % (len(allRoutes.all), allRoutes.number))
        if options.verboseHistogram:
            ...
            for i, edges in enumerate(allRoutes.unique):
    ...
    with open(options.out, 'w') as outf:
        ...
        for begin, end in intervals:
            ...
            uFlow, oFlow, gehOK = solveInterval(options, routesSeries.getRoutesForInterval(begin, end), begin, end, intervalPrefix, outf, mismatchf)
...
def solveInterval(options, routes, begin, end, intervalPrefix, outf, mismatchf):
    ...
    if options.optimizeInput:
        ...
        resetCounts(usedRoutes, routeUsage, countData)  # set to original counts
    else:
        while openCounts:
    ...
    if abs(totalMismatch) > 0 and options.optimize is not None:  # FIXME: this does not optimize undercounts
        if options.verbose:
            ...
            for i, routeIndex in enumerate(usedRoutes):
                ...
                if routeID is not None:
                    outf.write('    <vehicle id="%s" depart="%.2f" route="%s%s"%s/>\n' % (
                        vehID, depart, intervalPrefix, routeID, options.vehattrs))  # FIXME: maintain vehicle types
                else:
    ...
    for cd in countData:
        ...
        geh = sumolib.miscutils.geh(origHourly, localHourly)  # TODO: check the GEH calculation
